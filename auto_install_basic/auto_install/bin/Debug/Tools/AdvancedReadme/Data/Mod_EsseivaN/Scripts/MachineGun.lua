...
-- Enable first person effect
local enableFP_Animation = false
local enableSpread = false
local disableSpreadOnAim = true

-- ######################## END OF THE FREE TO EDIT REGION #################################

-- real firerate (per second) = 40 / fireRate
local fireRate = 40
...
function MachineGun.client_onCreate( self )
	...
	self.fireRateCounter = fireRate
end



function MachineGun.client_onRefresh( self )
	self:loadAnimations()
end

function MachineGun.loadAnimations( self )
...
function MachineGun.client_onUpdate( self, dt )
...
function MachineGun.client_onEquip( self )
...
function MachineGun.client_onUnequip( self )
	self.equipped = false
...
function MachineGun.server_network_onAim( self, aiming )
	self.network:sendToClients( "client_network_onAim", aiming )
end

function MachineGun.client_network_onAim( self, aiming )
	if not self.tool:isLocal() then
		self:onAim( aiming )
	end
end

function MachineGun.onAim( self, aiming )
	self.aiming = aiming
...
function MachineGun.server_network_onShoot( self, dir ) 
	self.network:sendToClients( "client_network_onShoot", dir )
end

function MachineGun.client_network_onShoot( self, dir ) 
	if not self.tool:isLocal() then
		self:onShoot( dir )
	end
end

function MachineGun.onShoot( self, dir ) 
	self.tpAnimations.animations.idle.time = 0
	self.tpAnimations.animations.shoot.time = 0
	self.tpAnimations.animations.aimShoot.time = 0

	setTpAnimation( self.tpAnimations, self.aiming and "aimShoot" or "shoot", 0.0 )
	
	if self.tool:isInFirstPersonView() then
			if enableFP_Animation then
...
function MachineGun.calculateFirePosition( self )
...
function MachineGun.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )		
	local right = sm.localPlayer.getRight()
...
function MachineGun.calculateFpMuzzlePos( self )
...
function MachineGun.client_onToggle( self, state )
	if fireRate >= 40 then
		fireRate = 1
	else
		fireRate = fireRate + 1
...
function MachineGun.client_onReload( self, state )
	if fireRate > 1 then
		fireRate = fireRate - 1
	else
		fireRate = 40
...
function MachineGun.client_onFixedUpdate( self, dt )
	if(self.primaryState == 1 or self.primaryState == 2) then
		if self.fireRateCounter >= fireRate then
			self.fireRateCounter = 0
			self:client_onPrimaryUse( self.primaryState )
		else
...
function MachineGun.client_onPrimaryUse( self, state )
	self.fireCooldownTimer = 0
	self.primaryState = state
	if self.fireCooldownTimer <= 0.0 and (state == 1 or state == 2) then
		...
		-- Spread
		local fireMode = self.aiming and self.aimFireMode or self.normalFireMode
		if enableSpread and not ( disableSpreadOnAim and self.aiming) then
			local recoilDispersion = 1.0 - ( math.max(fireMode.minDispersionCrouching, fireMode.minDispersionStanding ) + fireMode.maxMovementDispersion )
...
function MachineGun.client_onSecondaryUse( self, state )
