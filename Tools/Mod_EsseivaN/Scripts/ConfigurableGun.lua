...
-- version 1
-- Author : EsseivaN
-- Description : Configurable Gun

dofile "AnimationUtil.lua"

ConfigurableGun = class()


-- modes :
-- 0 : 8 projectiles shotgun
-- 1 : 32 projectiles shotgun
-- 2 : Repulsor gun (low power)
-- 3 : Repulsor gun (extra power)

local normalShotgun1FireMode = {
		fireCooldown = 0.250,
		spreadCooldown = 0,
		spreadIncrement = 0,
		spreadMinAngle = 5,
...
local normalRepulsor1FireMode = {
		fireCooldown = 0,
		spreadCooldown = 0,
		spreadIncrement = 0,
		spreadMinAngle = 0,
		spreadMaxAngle = 0,
		fireVelocity = 99999.0,
...
local normalRepulsor2FireMode = {
		fireCooldown = 2.0,
		spreadCooldown = 0,
		spreadIncrement = 0,
		spreadMinAngle = 0,
		spreadMaxAngle = 0,
		fireVelocity = 9999999.0,
...
function ConfigurableGun.client_onCreate( self )
...
function ConfigurableGun.client_onReload( self )
	if (self.mode < maxMode) then
...
function ConfigurableGun.client_onToggle( self )
	if (self.mode > 0) then
		self.mode = self.mode - 1
	else
		self.mode = maxMode
...
function ConfigurableGun.updateFireMode( self )
	local fireMode = (self.aiming and aimShotgun1FireMode or normalShotgun1FireMode)
	if (self.mode == 0) then
		print("Shotgun x8")
		fireMode = (self.aiming and aimShotgun1FireMode or normalShotgun1FireMode)
	elseif (self.mode == 1) then
		print("Shotgun x32")
		fireMode = (self.aiming and aimShotgun1FireMode or normalShotgun1FireMode)
	elseif (self.mode == 2) then
		print("Repulsor low")
		fireMode = normalRepulsor1FireMode
	elseif (self.mode == 3) then
		print("Repulsor extra (warning using this)")
		fireMode = normalRepulsor2FireMode
	end
	self.fireMode = fireMode
end

function ConfigurableGun.client_onRefresh( self )
	self:loadAnimations()
end

function ConfigurableGun.loadAnimations( self )
...
function ConfigurableGun.client_onUpdate( self, dt )
...
function ConfigurableGun.client_onEquip( self )
...
function ConfigurableGun.client_onUnequip( self )
	self.equipped = false
...
function ConfigurableGun.server_network_onAim( self, aiming )
	self.network:sendToClients( "client_network_onAim", aiming )
end

function ConfigurableGun.client_network_onAim( self, aiming )
	if not self.tool:isLocal() then
		self:onAim( aiming )
	end
end

function ConfigurableGun.onAim( self, aiming )
	self.aiming = aiming
	self:updateFireMode()
...
function ConfigurableGun.server_network_onShoot( self, dir ) 
	self.network:sendToClients( "client_network_onShoot", dir )
end

function ConfigurableGun.client_network_onShoot( self, dir ) 
	if not self.tool:isLocal() then
		self:onShoot( dir )
	end
end

function ConfigurableGun.onShoot( self, dir ) 
...
function ConfigurableGun.calculateFirePosition( self )
...
function ConfigurableGun.calculateTpMuzzlePos( self )
	local crouching = self.tool:isCrouching()
	local dir = sm.localPlayer.getDirection()
	local pitch = math.asin( dir.z )		
	local right = sm.localPlayer.getRight()
...
function ConfigurableGun.calculateFpMuzzlePos( self )
...
function ConfigurableGun.client_onPrimaryUse( self, state )
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		self:FireProjectile(count, self.fireMode)
	end
end

function ConfigurableGun.FireProjectile( self, count, fireMode )
...
function ConfigurableGun.client_onSecondaryUse( self, state )
