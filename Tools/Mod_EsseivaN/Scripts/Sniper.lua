...
function Sniper.loadAnimations( self )
	...
	self.normalFireMode = {
		...
		minDispersionStanding = 0.2,
		minDispersionCrouching = 0.08,
		
		maxMovementDispersion = 0.8,
		jumpDispersionMultiplier = 32
	}
	...
	self.aimFireMode = {
		...
		minDispersionCrouching = 0.005,
		
		maxMovementDispersion = 0.4,
		jumpDispersionMultiplier = 16
	}
...
function Sniper.client_onUpdate( self, dt )
	...
	self.tool:updateFpCamera( 0.0, sm.vec3.new( 0.0, 0.0, 0.0 ), self.aimWeight, bobbing )
end

function Sniper.client_onEquip( self )
...
function Sniper.client_onUnequip( self )
	self.equipped = false
...
function Sniper.server_network_onAim( self, aiming )
	self.network:sendToClients( "client_network_onAim", aiming )
end

function Sniper.client_network_onAim( self, aiming )
...
function Sniper.server_network_onShoot( self, dir ) 
	self.network:sendToClients( "client_network_onShoot", dir )
end

function Sniper.client_network_onShoot( self, dir ) 
	if not self.tool:isLocal() then
		self:onShoot( dir )
	end
end
...
function Sniper.client_onPrimaryUse( self, state )
	if self.fireCooldownTimer <= 0.0 and state == sm.tool.interactState.start then
		...
		dir = dir:rotate( math.rad( 0.20 ), sm.camera.getRight() ) -- 50 m sight calibration
...
function Sniper.client_onSecondaryUse( self, state )
